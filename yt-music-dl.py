    ...
    try:
        ...
        if isinstance(info, dict) and info.get('entries'):
            ...
            for i, entry in enumerate(entries, 1):
                ...
                process_song(video_url, folder, i, len(entries), include_meta, include_cover, include_lyrics)
        else:
...
def main():
    ...
    for i, url in enumerate(urls, 1):
        console.rule(f"[bold green]Processing {i}/{len(urls)}[/bold green]")
        try:
            process_url(url, output_dir, include_meta, include_cover, include_lyrics)
        except Exception as e:
